...
//--------------------------------------
  // SCRIPTS FOR THE PRODUCT'S CHECKBOXES
    // Asegurarse de que el DOM esté completamente cargado antes de añadir event listeners
    document.addEventListener('DOMContentLoaded', function() {  
      
    // Función para insertar casillas de verificación 
    function insertCheckboxes() { 
      document.querySelectorAll('.checkbox-placeholder')
      .forEach((placeholder) => { const label = document.createElement('label'); 
      label.innerHTML = `<input type="checkbox" class="productCheckbox">`; placeholder.appendChild(label); }); } 
      
    // Llamada a la función para insertar casillas de verificación 
    insertCheckboxes();       
    // Lógica para habilitar/deshabilitar el botón "Comprar Ahora" basado en las casillas de verificación 
    document.querySelectorAll('.productCheckbox')
    .forEach(checkbox => { checkbox.addEventListener('change', function() {
  ...
  // END OF SCRIPTS FOR THE PRODUCT'S CHECKBOXES
  //-------------------------------------
  //-------------------------------------
